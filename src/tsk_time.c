...
/* For windows implementation of "gettimeofday" Thanks to
 ...
 * The tsk_gettimeofday() function shall obtain the current time, expressed as
 * seconds and microseconds since EPOCH (00:00:00 UTC on 1 January 1970). The
 * resolution of the system clock is unspecified.
 * @param tv The current time, expressed as seconds and microseconds since
 * EPOCH(00:00:00 UTC on 1 January 1970).
 * @param tz The timezone.
 * @retval The tsk_gettimeofday() function shall return 0 and no value shall be
...
uint64_t tsk_time_now() {
    ...
    if (__cpu_count == 1) {
        ...
        return (uint64_t)(((double)liPerformanceCount.QuadPart /
                           (double)__liFrequency.QuadPart) *
                          1000.0);
    } else {
#if TSK_UNDER_WINDOWS_RT
    ...
    return (((uint64_t)ts.tv_sec) * (uint64_t)1000) +
    ...
    return (uint64_t)((mach_absolute_time() * __apple_timebase_info.numer) /
                      (1e+6 * __apple_timebase_info.denom));
#else
    struct timeval tv;
...
#else
    tv_usecs = ((uint64_t)tv->tv_usec * (uint64_t)__ntp_scale_frac) /
               (uint64_t)1000000;
